# --- START OF FILE server_websocket2.py (FINAL + IMAGE SAVING) ---
import asyncio
import cv2
import json
import time
import logging
import os # --- NEW --- Needed for creating directories
...
# --- NEW --- Directory for saving image captures
CAPTURES_DIR = STATIC_FILES_DIR / "captures"
...
executor = concurrent.futures.ThreadPoolExecutor(max_workers=2, thread_name_prefix='ProcWorker')
demo_image = None
CURRENT_SOURCE = "static_video"

# (CameraManager and ThermalCameraManager classes remain unchanged)
class CameraManager:
    ...
    def read(self):
        ...
        try:
            if self._is_picam_active:
                ...
                return cv2.cvtColor(frame_rgb, cv2.COLOR_RGB_BGR)
            else:
...
# --- NEW --- Helper function to save cropped chicken images
def save_chicken_capture(original_frame, bbox_xywh, filename_prefix):
    """Crops a chicken from the original frame and saves it as a JPEG."""
    try:
        h_orig, w_orig, _ = original_frame.shape
        x, y, w, h = bbox_xywh
        
        # Scale bbox from resized coordinates to original frame coordinates
        x_orig, y_orig = int(x * w_orig / FRAME_WIDTH), int(y * h_orig / FRAME_HEIGHT)
        w_orig, h_orig = int(w * w_orig / FRAME_WIDTH), int(h * h_orig / FRAME_HEIGHT)
        
        # Add some padding to the crop
        padding = 20
        crop_x1 = max(0, x_orig - padding)
        crop_y1 = max(0, y_orig - padding)
        crop_x2 = min(original_frame.shape[1], x_orig + w_orig + padding)
        crop_y2 = min(original_frame.shape[0], y_orig + h_orig + padding)
        
        chicken_crop = original_frame[crop_y1:crop_y2, crop_x1:crop_x2]
        
        if chicken_crop.size > 0:
            full_path = CAPTURES_DIR / f"{filename_prefix}.jpg"
            cv2.imwrite(str(full_path), chicken_crop)
            logger.info(f"Saved capture: {full_path.name}")
            
    except Exception as e:
        logger.error(f"Failed to save chicken capture: {e}", exc_info=True)
...
def _process_frame_yolo_tracking_blocking(frame_bgr_original, frame_bgr_resized, frame_idx):
    ...
    for det_idx in unmatched_detection_indices:
        ...
        temp_tracked_objects_next_state[next_track_id] = {
            ...
            "movement_history": [], "lethargy_start_time": 0, "classification_count": 0
        }
        ...
        for track in current_tracks_output:
            ...
            if (frame_idx - data["last_classified_frame"] > CLASSIFICATION_INTERVAL_FRAMES) and \
               ...
               (data.get("classification_count", 0) < MAX_CLASSIFICATION_COUNT):
                ...
                # --- NEW --- Logic for WARNING capture
                # Check if the history was "clean" before this scan
                was_clean_before_scan = all(d == "Low Confidence" for d in data.get("disease_history", []))

                data["last_classified_frame"] = frame_idx
                h_orig, w_orig, _ = frame_bgr_original.shape
                x, y, w, h = track["bbox_xywh"]
                x_orig_crop, y_orig_crop = int(x * h_orig / FRAME_HEIGHT), int(y * w_orig / FRAME_WIDTH) # Corrected scaling
                w_orig_crop, h_orig_crop = int(w * h_orig / FRAME_HEIGHT), int(h * w_orig / FRAME_WIDTH)
                
                padding = 15
                crop_x1 = max(0, x_orig_crop - padding)
                crop_y1 = max(0, y_orig_crop - padding)
                crop_x2 = min(frame_bgr_original.shape[1], x_orig_crop + w_orig_crop + padding)
                crop_y2 = min(frame_bgr_original.shape[0], y_orig_crop + h_orig_crop + padding)
                ...
                if chicken_crop.size > 0:
                    ...
                    if disease:
                        ...
                        data["disease_history"] = data["disease_history"][-DISEASE_HISTORY_LENGTH:]

                        # --- NEW --- If it was clean before and now has a valid disease, save a WARNING image
                        if was_clean_before_scan and disease != "Low Confidence":
                            timestamp = time.strftime("%Y%m%d-%H%M%S")
                            filename = f"{timestamp}_track{track_id}_{disease}_WARNING"
                            save_chicken_capture(frame_bgr_original, track["bbox_xywh"], filename)
    ...
    for track in current_tracks_output:
        ...
        # --- NEW --- Logic for ALERT capture
        # Check for the flag set by analyze_anomalies
        if data.get("save_alert_capture"):
            timestamp = time.strftime("%Y%m%d-%H%M%S")
            disease_name = data.get("active_alert", {}).get("disease", "Lethargy")
            filename = f"{timestamp}_track{track_id}_{disease_name}_ALERT"
            save_chicken_capture(frame_bgr_original, track["bbox_xywh"], filename)
            # Remove the flag to ensure we only save once per alert
            del tracked_objects[track_id]["save_alert_capture"]
            ...
            if data.get("active_alert") is None and data["disease_history"]:
                ...
                if disease_only_history:
                    ...
                    if count >= DISEASE_ALERT_THRESHOLD_COUNT:
                        ...
                        # --- NEW --- Set the flag to save an ALERT image on the next frame
                        tracked_objects[track_id]["save_alert_capture"] = True
        ...
        if data.get("active_alert") is None and len(data["movement_history"]) > 0:
            ...
            if is_lethargic:
                if data["lethargy_start_time"] == 0:
                    ...
                    # --- NEW --- Set the flag to save an ALERT image for lethargy
                    tracked_objects[track_id]["save_alert_capture"] = True
                    logger.warning(f"ALERT TRIGGERED: {alert['message']}")
            else:
...
async def handle_websocket_message(ws, addr, message_str):
    ...
    try:
        ...
        if action == "set_source":
            ...
            if source in ["static_video", "demo_image", "normal_video"]:
...
async def on_app_startup(app_instance):
    ...
    # --- NEW --- Create the captures directory on startup
    os.makedirs(CAPTURES_DIR, exist_ok=True)
    logger.info(f"Image captures will be saved to: {CAPTURES_DIR}")
